...
- Open questions that do not block implementation: Final public brand and domain; hosting provider; collection cadence after the MVP; business traffic threshold; monetization providers; and long-term retention periods for noncanonical operational logs. The public GitHub repository, baseline, PostgreSQL 17, uv, and npm were approved on 2026-08-29. Live external acceptance remains gated on human approval of the chosen source interface and its terms, but deterministic local implementation can proceed before that checkpoint.
