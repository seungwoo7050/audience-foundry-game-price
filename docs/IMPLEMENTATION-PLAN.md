...
| 1 | Is the MVP decomposed into reviewable, evidence-backed changes? | This plan only | Required fields complete; no unresolved required placeholder | Revert this documentation commit | in progress |
| 2 | Can both applications install reproducibly on the approved runtimes? | `backend/pyproject.toml`, `backend/uv.lock`, `web/package.json`, `web/package-lock.json`, runtime pins, `THIRD_PARTY_NOTICES.md` | Frozen installs; version and license metadata; advisory checks | Remove manifests, locks, and installed environments | planned |
